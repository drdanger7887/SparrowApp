...
//image tag
function theme_advertisement_img($uri,$size='small')
{
	
		$CI =& get_instance();
		return $CI->config->base_url('uploads/advertisement/'.$size.'/'.$uri);
}
...
function theme_comport_img($uri) {

    $CI = & get_instance();
    return $CI->config->base_url('uploads/comport/thumbnails/' . $uri);
}
