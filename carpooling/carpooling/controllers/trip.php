...
class Trip extends Front_Controller {
    ...
    function tripconfirm($id) {  ///// passenger confirm Journey
...
/**/
    ...
    function tripdetails($id) {
        ...
        $data['money_for_journey'] = $this->Money_model->money_for_journey;
        $data['linkledid'] = $id;
        ...
        if (!empty($data['tripdetails'])) {
            if (!empty($this->user['user_id'])) {
                ...
                $data['already'] = $this->check_enquiry_already($this->user['user_id'], $data['tripdetails']['trip_id']);
            }
            ...
            $this->session->set_flashdata('error', 'The journey not found');
            redirect('community');
        }
    }
    ...
    function check_enquiry_already($user_id, $trip_id) {
