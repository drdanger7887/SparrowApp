...
class Addtrip extends Traveller_Controller {
    ...
    function form($trip_id = false, $ajax = false) {
        ...
        } else if ($this->input->post('communityid',true)) {
            $data['communityid'] = $this->input->post('communityid');
        } else {$data['communityid'] = 1;}
        
        $communityslug = $this->community_model->get_community($data['communityid']);
        if (!$communityslug) {
             $this->session->set_flashdata('error', 'Select a Community!');
            redirect('community');
        } else {
            $communityslug = $communityslug[0]->comm_slug;
        }
        ...
        if ($this->input->get('r', true)) {
            ...
            $this->db->where('tbl_request_trips.trip_id',$this->input->get('r'));
            $query = $this->db->get('tbl_request_trips');      
            $rqresult = $query->result_array();

            if($rqresult) {
                $data['seo_title'] = 'Add your journey for Request';

                $request_time = date("d/m/Y g:i a",$rqresult[0]['trip_unix_time']);
                //echo $request_time;
                //var_dump($rqresult);
                $fresult = explode(' ', $request_time);
                $data['rpt_from_date'] = $fresult[0];
                $ftime = explode(':', $fresult[1]);
                
                $data['fhh'] = $ftime[0];
                $data['fmm'] = $ftime[1];
                $data['fzone'] = $fresult[2];

                $data['txtsource'] = $rqresult[0]['trip_source'];
                $data['source_ids'] = $rqresult[0]['trip_from_latlan'];

                $data['txtdestination'] = $rqresult[0]['trip_destination'];
                $data['destination_ids'] = $rqresult[0]['trip_to_latlan'];

                $data['request'] = $this->input->get('r');
                $data['requester_id'] = $rqresult[0]['trip_user_id'];


            } else {
                $this->session->set_flashdata('error', 'Wrong Request');
                redirect('community/site/'.$communityslug);
            }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //   Saving Trip
             //file_put_contents('log.txt', $this->input->post('fhh').'---');
            //die();     
            ...
            /////// Geka Save trip
            $save['trip_community'] = $data['communityid']; 
            $trip_date = str_replace('/', '-', $save['trip_casual_date']);
            $trip_datetime = $trip_date.' '.$save['trip_depature_time'];
            $save['trip_unix_time'] = strtotime($trip_datetime);

            if (($save['trip_unix_time']-3600)<time()) {
                die(json_encode(array('result' => false, 'error' => 'Sorry, you can only post a journey up to one hour of scheduled time.')));
                //die(json_encode(array('result' => false, 'error' => 'Sorry, you can only post a journey up to one hour of scheduled time. Now: '.date('Y-m-d g:i a',time()).' requested: '.date('Y-m-d g:i a',$save['trip_unix_time']-3600))));
            } 
            ...
            $logdata['user']=$this->user_id;
            $logdata['action']='saved trip #'.$trip_id;
            if ($this->input->post('request_id') != '') {$logdata['action']='save trip #'.$trip_id.' for request #'.$this->input->post('request_id');}
            logAll($logdata);

            ////////  If Post for Require
            if ($this->input->post('request_id') != '') {  

                $save_bank['bank_transfer']='trip';
                $save_bank['bank_transfer_id']=$trip_id;
                $this->db->where('tbl_bank.bank_transfer','request');
                $this->db->where('tbl_bank.bank_transfer_id',$this->input->post('request_id'));
                $this->db->update('tbl_bank', $save_bank);  


                $this->db->where('trip_id', $this->input->post('request_id'));
                $this->db->delete('tbl_request_trips');

                $enquire_data['enquiry_passanger_id']=$this->input->post('requester_id');
                $enquire_data['enquiry_trip_id']=$trip_id;
                $enquire_data['enquire_travel_id']=$save['trip_user_id'];
                $enquire_data['enquiry_trip_date']=$trip_date;
                $enquire_data['enquiry_trip_status']='1';
                $enquire_data['payment_flg']='0';
                $enquire_data['enquiry_trip_unix_time']=$save['trip_unix_time'];
                $enquire_data['enquiry_cron_checked']='0';
                $this->db->insert('tbl_enquires', $enquire_data);
            } 
                //file_put_contents('log.txt', $trip_id.'-!!!!---');
...
//------------------------------------ trip leg concept ------------------------------------------------------------------------
            ...
            if ($ajax) {
                $this->session->set_flashdata('message', 'Your Trip Has Been Added');  ////  This message works
    ...
    function form_request() {
        ...
        $data['seo_title'] = 'Add your request here';
        $data['seo_description'] = '';
        $data['seo_keyword'] = '';
        ...
        if ($this->input->post('submitted')) {   ////   REQUEST post
           
            $save['trip_source'] = $this->input->post('txtsource');  // from
            $save['trip_from_latlan'] = $this->input->post('source_ids');
            $save['trip_destination'] = $this->input->post('txtdestination');  // to
            $save['trip_to_latlan'] = $this->input->post('destination_ids');

            $rqtime = date('Y-m-d', strtotime(str_replace("/", "-", $this->input->post('rpt_from_date'))));
            $rqtime .= " ".$this->input->post('fhh').":".$this->input->post('fmm')." ".$this->input->post('fzone');
            $save['trip_unix_time'] = strtotime($rqtime);

            $save['trip_comments'] = strip_tags($this->input->post('comments'));

            $this->CI = & get_instance();
            $carpool_session['carpool_session'] = $this->CI->carpool_session->userdata('carpool');
            $save['trip_user_id'] = $carpool_session['carpool_session']['user_id'];
            $save['trip_community'] = $data['communityid'];
//echo $rqtime;
//var_dump($save);
            if ($save['trip_from_latlan']!="" && $save['trip_to_latlan']!="" && $save['trip_unix_time']!="") {



                
                if (($save['trip_unix_time']-7200)<time()) {

 //die(json_encode(array('result' => false, 'error' => 'Sorry, you can only post a journey up to one hour of scheduled time. Now: '.date('Y-m-d g:i a',time()).' requested: '.date('Y-m-d g:i a',$save['trip_unix_time']-3600))));

                    $this->session->set_flashdata('error', 'Sorry, you can only request a journey up to two hours of scheduled time');
                    redirect('addtrip/form_request?commid='.$data['communityid']);
                } else {
                    //  Save request
                    if ($this->Money_model->change_usermoney($save['trip_user_id'],-$this->Money_model->money_for_journey)) {
                        $this->db->insert('tbl_request_trips', $save);
                        $request_id = $this->db->insert_id();

                        $this->Money_model->reserve_usermoney($save['trip_user_id'],$this->Money_model->money_for_journey,$this->db->insert_id(),'request');

                        $logdata['user']=$save['trip_user_id'];
                        $logdata['action']='add request #'.$request_id;
                        logAll($logdata);

                        $this->session->set_flashdata('message', 'Your Request Has Been Added');
                        redirect('community/site/'.$communityslug);
                    } else {
                        $this->session->set_flashdata('error', 'Sorry, you don&#39;t have enough points');
                        redirect('addtrip/form_request?commid='.$data['communityid']);                       
                    }
                }
            } else {
            ...
            $this->load->view('addrequesttrip', $data);
        }
    }
    ...
    function delete($id = 0) {
        if ($id) {
            $this->Trip_model->delete_trip_by_edit($id);   
                ...
                $logdata['user']=$this->user_id;
                $logdata['action']='delete trip #'.$id;
                logAll($logdata);

            $this->session->set_flashdata('message', 'Your Journey has been deleted');
            redirect('addtrip');
        }
    }

    function delete_request($id = 0) {
        if ($id) {
           
            $this->db->where('tbl_request_trips.trip_id', $id);
            $result = $this->db->get('tbl_request_trips');
            $result = $result->result_array();
            //var_dump($result);  
            $carpool_session['carpool_session'] = $this->CI->carpool_session->userdata('carpool');
            $this->user_id = $carpool_session['carpool_session']['user_id'];
            //echo $result[0]['trip_user_id'].' - '.$this->user_id;
            if ($result && $result[0]['trip_user_id']==$this->user_id) {
                $this->db->where('tbl_bank.bank_user_id', $this->user_id);
                $this->db->where('tbl_bank.bank_transfer', 'request');
                $this->db->where('tbl_bank.bank_transfer_id', $id);
                $result_b = $this->db->get('tbl_bank');
                $result_b = $result_b->result_array();
                //var_dump($result_b);
                if ($result_b) {
                    ...
                    $this->Money_model->change_usermoney($this->user_id,$result_b[0]['bank_count']);

                    $this->session->set_flashdata('message', 'Your Request has been deleted');

                    $logdata['user']=$this->user_id;
                    $logdata['action']='delete request #'.$id;
                    logAll($logdata); 

                } else {
                    $this->session->set_flashdata('error', 'Moneyback error');
                }
                   
                    

            } else {
                $this->session->set_flashdata('error', 'Wrong Request id');
            }
            
            redirect('addtrip/upcoming_trip_requests');
    ...
    function upcoming_trip() {
        ...
        $carpool_session['carpool_session'] = $this->CI->carpool_session->userdata('carpool');
        $this->user_id = $carpool_session['carpool_session']['user_id'];
        $data['seo_title'] = 'My trips as a car owner';
        $data['seo_description'] = ''; 
        $data['seo_keyword'] = '';
        ...
        $data = $this->Trip_model->get_trips_geka($this->user_id, $data);         ///  Geka
    ...
    function upcoming_trip_passenger() {
        ...
        $carpool_session['carpool_session'] = $this->CI->carpool_session->userdata('carpool');
        $this->user_id = $carpool_session['carpool_session']['user_id'];
        $data['seo_title'] = 'My trips as a passenger';
        $data['seo_description'] = '';
        $data['seo_keyword'] = '';
        $data['menuactive'] = 2;
...
///  Geka
    function upcoming_trip_requests() {

        $data = array();
        $this->load->helper('form');
        $carpool_session['carpool_session'] = $this->CI->carpool_session->userdata('carpool');
        $this->user_id = $carpool_session['carpool_session']['user_id'];
        $data['seo_title'] = 'My Requests';
        $data['seo_description'] = '';
        $data['seo_keyword'] = '';
        $data['menuactive'] = 3;
        $data['rq'] = $this->Trip_model->get_requests_community('','','future',$this->user_id);
        //var_dump($data);
        $this->load->view('trips_requests', $data);
    }    
