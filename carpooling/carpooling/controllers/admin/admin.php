...
class Admin extends Admin_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('country_model', 'currency_model', 'language_model', 'logo_model', 'payment_model', 'social_model'));
    ...
    function edit_profile($id = false) {
        ...
        $this->form_validation->set_rules('company_name', 'Comapany Name', 'trim|max_length[32]|required');
        $this->form_validation->set_rules('first_name', 'lang:firstname', 'trim|max_length[32]|required');
        $this->form_validation->set_rules('last_name', 'lang:lastname', 'trim|max_length[32|required]');
        $this->form_validation->set_rules('company_email', 'lang:email', 'trim|valid_email|max_length[128]|callback_check_email|required');
        $this->form_validation->set_rules('company_mobile', 'Company mobile', 'trim|max_length[10]|min_length[10]|numeric|required');
    ...
    function edit_settings() {
        ...
        $data['user_language'] = $settings['user_language'];
        ...
        $data['payment_commission'] = $settings['payment_commission'];
        $data['payment_status'] = $settings['payment_status'];
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $settings['user_language'] = (implode(',', $this->input->post('user_language')));
            ...
            $settings['payment_commission'] = $this->input->post('payment_commission');
            $settings['payment_status'] = $this->input->post('payment_status');
            $settings['google_api_key'] = $this->input->post('google_api_key');
    ...
    function change_logo($id = false) {
        ...
        if ($id) {
            ...
            //set values to db values
            $data['id'] = $admin->id;
            $data['uploadvalues'] = $admin->uploadvalues;
        }
    ...
    function payment() {
        ...
        if ($submitted) {
            $paymentDetails = $this->input->post('payment');
            if ($paymentDetails) {
                foreach ($paymentDetails as $key => $row) {
                    $param['id'] = $key;
                    $param['payment_id'] = $row['paymentID'];
                    $param['is_active'] = $row['paymentActive'];
                    $param['is_method'] = $row['paymentMethod'];

                    $this->payment_model->save($param);
                }
            }
    ...
    function social_media() {
        ...
        $data['socialmedias'] = $this->social_model->get_all_social_media();
        ...
        if ($submitted) {
            ...
            $socialmediaDetails = $this->input->post('social_media');

            if ($socialmediaDetails) {
                foreach ($socialmediaDetails as $key => $row) {
                    $param['id'] = $key;
                    $param['page_url'] = $row['page_url'];
                  
                    $this->social_model->save($param);
                }
            }

            redirect('admin/dashboard');
        }
        $this->load->view($this->config->item('admin_folder') . '/social_media_form', $data);
    }
