         ...
         // var_dump($result);
          for($i=0;$i<sizeof($result);$i++) {
              ...
              if ($_SERVER['SERVER_NAME']!="localhost") {
                $this->sendmail_confirm($result[$i],$result_user[0]);
              }
          }
...
echo '</body>
    ...
    function sendmail_confirm($result,$result_user) {

        $res = $this->db->where('tplid', '38')->get('tbl_email_template');
        $row = $res->row_array();
        ...
        $row['tplsubject'] = str_replace('{TRIPID}', $result['enquiry_trip_id'], $row['tplsubject']);
        $row['tplmessage'] = str_replace('{NAME}', $result_user['user_first_name'], $row['tplmessage']);
        $row['tplmessage'] = str_replace('{COMPANY_NAME}', $this->config->item('company_name'), $row['tplmessage']);
        $row['tplmessage'] = str_replace('{TRIPTIME}', $result['enquiry_trip_date'], $row['tplmessage']);
        $row['tplmessage'] = str_replace('{TRIPLINK}', base_url('community/trip')."/".$result['enquiry_trip_id']."#confirm", $row['tplmessage']);
