...
////			// get the email template
            $res = $this->db->where('tplid', '1')->get('tbl_email_template');
            $row = $res->row_array();
	    ...
	    $row['tplmessage'] = str_replace('{COMPANY_NAME}', $this->config->item('company_name'), $row['tplmessage']);
            ...
            $this->session->set_flashdata('message', 'Thanks for joining Sparrow journey! Check your email to verify your account.');
            redirect($redirect);
        }
    }
