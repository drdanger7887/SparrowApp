...
/*   Geka  show user money */

	function show_usermoney($uid)
	{
		$this->CI->db->select('*');
		$this->CI->db->where('tbl_money.money_user',$uid);
        $query = $this->CI->db->get('tbl_money');
        $result = $query->result_array();
        if (!$result) {
        	    $tblmoney['money_user'] = $uid;
                $tblmoney['money_count'] = 3; // Geka default money Starting capital
                $this->CI->db->insert('tbl_money', $tblmoney);
				$this->CI->db->select('*');
				$this->CI->db->where('tbl_money.money_user',$uid);
        		$query = $this->CI->db->get('tbl_money');
        		$result = $query->result_array();
