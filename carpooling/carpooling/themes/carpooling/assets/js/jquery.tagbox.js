...
(function(jQuery) {
  jQuery.fn.tagBox = function(options) {
    var defaults = {
      ...
      tagButtonClassName: 'alinkbtn alinkbtn1 size14 padding10 fright',
      tagButtonTitle: 'Add Destination',
...
//  Hide Element
  ...
  //  Create DOM Elements
      if( (options.enableDropdown) && options.dropdownSource() != null ) {
        ...
        var $tag_input_elem = (options.readonly) ? '' : jQuery('<input type="text" class="row padding10 row59 '+options.className+'-input" value="" id="welcome" placeholder=" "/>');
      }
      ...
      if(!options.readonly) {
        $tag_add_elem.click(function() {
			...
			setTimeout(function() {
			  ...
			  //valu = value.split(',');
			  //var vals = '';
			  var source_ids = document.getElementById("source_ids").value;
        if (source_ids=="") {
        alert('Please fill your departure point');
        return false;
        }
        else {
          if(value!=''){  
